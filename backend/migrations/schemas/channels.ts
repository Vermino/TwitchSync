...
export const channelsQueries = {
  ...
  getAllChannels: async (pool: Pool): Promise<Channel[]> => {
    const result = await pool.query<Channel>('SELECT * FROM channels ORDER BY created_at DESC');
    return result.rows;
  },

  getChannelById: async (pool: Pool, id: number): Promise<Channel | null> => {
    const result = await pool.query<Channel>('SELECT * FROM channels WHERE id = $1', [id]);
    return result.rows[0] || null;
  },

  createChannel: async (pool: Pool, data: Omit<Channel, 'id' | 'created_at'>): Promise<Channel> => {
  ...
  updateChannel: async (pool: Pool, id: number, data: Partial<Channel>): Promise<Channel | null> => {
  ...
  updateLastChecks: async (pool: Pool, id: number, type: 'vod' | 'game'): Promise<void> => {
