...
export function setupQueueRoutes(pool: Pool): Router {
  ...
  router.get('/history', authenticate(pool), async (req: Request, res: Response) => {
    ...
    try {
      ...
      const statusFilter = status ? String(status).split(',') : ['completed', 'failed', 'cancelled'];
  ...
  // Delete a single VOD from download history
  router.delete('/history/:id', authenticate(pool), async (req: Request, res: Response) => {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
    ...
    try {
      const vodId = parseInt(req.params.id, 10);
      const deleted = await deleteHistoryItem(pool, vodId, Number(userId));
