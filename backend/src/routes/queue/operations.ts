...
export async function getQueueHistory(
  pool: Pool,
  userId: number,
  statusFilter: string[] = ['completed', 'failed', 'cancelled'],
  limit: number = 50,
  offset: number = 0,
  taskId?: number
): Promise<QueueResponse> {
...
/**
 * Delete a single history item that belongs to the user.
 */
  ...
  return withTransaction(pool, async (client) => {
    const result = await client.query(`
      ...
      WHERE vods.task_id = t.id
        AND vods.id = $1
        AND t.user_id = $2
        AND vods.download_status IN ('completed', 'failed', 'cancelled')
      RETURNING vods.id
    `, [vodId, userId]);
