...
export class TasksController {
  constructor(private operations: TaskOperations) {
    ...
    this.getTaskPerformance = this.getTaskPerformance.bind(this);
    this.getTaskAlerts = this.getTaskAlerts.bind(this);
  ...
  async getTaskStats(req: Request, res: Response) {
    try {
      ...
      const stats = await this.operations.getTaskMonitoringStats(taskId, Number(userId));
      res.json(stats);
    } catch (err) {
  ...
  async getTaskPerformance(req: Request, res: Response) {
    try {
      const userId = req.user?.id;
      const taskId = parseInt(req.params.id);
      const timeRange = ['hour', 'day', 'week', 'month'].includes(String(req.query.timeRange))
        ? String(req.query.timeRange) as 'hour' | 'day' | 'week' | 'month'
        : 'day';
      ...
      const performance = await this.operations.getTaskPerformanceHistory(taskId, Number(userId), timeRange);
      res.json(performance);
    } catch (err) {
      logger.error('Error fetching task performance history:', err);
      const error = err as Error;
      if (error.message === 'Task not found or access denied') {
        res.status(404).json({ error: 'Task not found' });
      } else {
        res.status(500).json({ error: 'Failed to fetch task performance history' });
      }
    }
  }

  async getTaskAlerts(req: Request, res: Response) {
    try {
      const userId = req.user?.id;
      const taskId = parseInt(req.params.id);
      ...
      const alerts = await this.operations.getTaskAlerts(taskId, Number(userId));
      res.json(alerts);
    } catch (err) {
      logger.error('Error fetching task alerts:', err);
      const error = err as Error;
      if (error.message === 'Task not found or access denied') {
        res.status(404).json({ error: 'Task not found' });
      } else {
  ...
  async manualRunTask(req: Request, res: Response) {
    try {
      ...
      const task = await this.operations.toggleTaskState(taskId, Number(userId), true);
