...
export function setupTaskRoutes(pool: Pool) {
  ...
  router.get('/:id/stats', authenticate(pool), controller.getTaskStats);
  router.get('/:id/performance', authenticate(pool), controller.getTaskPerformance);
  router.get('/:id/alerts', authenticate(pool), controller.getTaskAlerts);
