  ...
  router.post('/queue/add', authenticate(pool), handleAsync(async (req, res) => {
    ...
    try {
      ...
      const vodResult = await client.query<VOD>(
        `SELECT v.id, v.channel_id, v.download_status, v.task_id, v.twitch_id
         FROM vods v
         JOIN tasks t ON v.task_id = t.id
         WHERE v.id = $1 AND t.user_id = $2`,
        [vodId, parseInt(userId)]
      );
      ...
      if (vodResult.rows[0].download_status === 'completed') {
        return res.status(409).json({
          error: 'VOD already downloaded',
          message: 'Use the redownload endpoint for completed VODs.',
          vodId
        });
      }
      ...
      await client.query(`
        ...
        SET status = $1,
            ...
            updated_at = NOW()
        WHERE task_id = $3
      `, ['downloading', 'Download queued manually', vodResult.rows[0].task_id]);
