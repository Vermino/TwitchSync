...
export function setupAuthRoutes(pool: Pool): Router {
  ...
  const redirectToFrontendCallback = (
    ...
    const redirectUrl = new URL('/auth/callback', `${getPublicAppUrl()}/`);
  ...
  const markCodeProcessed = (code: string) => {
  ...
  const getOAuthErrorDetails = (error: any) => {
    ...
    if ((statusCode === 401 || statusCode === 403) && providerMessage.includes('invalid client secret')) {
      return {
        error: 'invalid_twitch_configuration',
        message: 'Twitch rejected the configured client secret. Update TWITCH_CLIENT_SECRET and try again.',
      };
    }

    if ((statusCode === 401 || statusCode === 403) && providerMessage.includes('invalid client id')) {
      return {
        error: 'invalid_twitch_configuration',
        message: 'Twitch rejected the configured client ID. Update TWITCH_CLIENT_ID and try again.',
      };
    }
    ...
    async (req, res) => {
      ...
      if (!code) {
        ...
        return redirectToFrontendCallback(res, {
          ...
          error_description: 'No authorization code was provided by Twitch.',
        });
      }
      ...
      if (processedCodes.has(String(code))) {
        ...
        return redirectToFrontendCallback(res, {
          ...
          error_description: 'This authorization code has already been used. Please start the login flow again.',
        });
      }
      ...
      try {
        ...
        const authError = getOAuthErrorDetails(error);
        return redirectToFrontendCallback(res, {
