...
const channelBaseSchema = {
  ...
  profile_image_url: z.string().url('Invalid profile image URL').or(z.literal('')).optional(),
...
export const CreateChannelSchema = z.object(channelBaseSchema);

// Update channel request schema (body only - params validated separately)
...
// Delete channel request schema (params only)
...
export const validateChannelData = (data: z.infer<typeof CreateChannelSchema>): void => {
  try {
