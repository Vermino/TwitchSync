...
// Trust reverse proxy (nginx, NPM, Cloudflare) — needed for correct
// req.protocol, req.ip, and secure cookie behaviour
...
const corsOptions: cors.CorsOptions = {
  origin: config.server.corsOrigin
    ? config.server.corsOrigin.split(',').map((o: string) => o.trim())
    : ['http://localhost:2261'],
  credentials: true,
