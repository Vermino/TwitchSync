...
export async function runMigrations(pool: Pool): Promise<void> {
  ...
  // In production, migrations are compiled to .js; in dev they're .ts
  const ext = __filename.endsWith('.js') ? '.js' : '.ts';
  const migrationFiles = fs.readdirSync(migrationsDir)
    .filter(file => file.endsWith(ext) && !file.includes('runner'))
    .sort();

  if (migrationFiles.length === 0) {
    logger.warn(`No migration files found (looking for *${ext}) in ${migrationsDir}`);
  }

  for (const file of migrationFiles) {
    // Normalize name for comparison — strip extension so .ts and .js records match
    const migrationName = file.replace(/\.(ts|js)$/, '');
    const alreadyRan = executedMigrations.some(m => m.replace(/\.(ts|js)$/, '') === migrationName);
