...
export const config = {
  ...
  server: serverConfigSchema.parse({
    ...
    // PUBLIC_URL is the single source of truth for the external URL.
    // FRONTEND_URL and CORS_ORIGIN fall back to it if not set explicitly.
    frontendUrl: process.env.FRONTEND_URL || process.env.PUBLIC_URL || 'http://localhost:2261',
    corsOrigin: process.env.CORS_ORIGIN || process.env.PUBLIC_URL || 'http://localhost:2261'
  }),
