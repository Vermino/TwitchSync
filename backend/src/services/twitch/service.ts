  ...
  async searchChannels(query: string): Promise<EnrichedChannel[]> {
    try {
      ...
      const followerEntries = await Promise.all(
        channels.map(async (channel) => [channel.id, await this.getChannelFollowers(channel.id)] as const)
      );
      const followerMap = new Map<string, number>(followerEntries);
      ...
      const enrichedChannels = await Promise.all(
        channels.map(async (channel) => {
          try {
            ...
            const followerCount = followerMap.get(channel.id) || 0;
            const displayName = channel.display_name || channel.login || channel.id;
            const username = channel.login || channel.display_name || channel.id;
            ...
            return {
              ...
              username,
              profile_image_url: profile?.profile_image_url || channel.profile_image_url,
              follower_count: followerMap.get(channel.id) || 0,
              is_live: false,
              tags: channel.tags || []
            };
          }
