  ...
  async searchChannels(query: string): Promise<TwitchChannel[]> {
    try {
      ...
      return response.data.map((channel: any) => ({
        ...
        login: channel.broadcaster_login || channel.user_login || channel.display_name || channel.id,
        display_name: channel.display_name || channel.user_name || channel.broadcaster_login || channel.user_login || channel.id,
