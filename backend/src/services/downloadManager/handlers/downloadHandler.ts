...
class DownloadCancelledError extends Error {
  constructor(vodId: number) {
    super(`Download cancelled for VOD ${vodId}`);
...
export class DownloadHandler extends EventEmitter {
  ...
  public requestCancellation(vodId: number): boolean {
    ...
    for (const resource of Array.from(state.openResources)) {
      try {
        resource.destroy(new DownloadCancelledError(vodId));
      } catch (error) {
        logger.debug(`Failed to destroy active resource while cancelling VOD ${vodId}:`, error);
      }
    }

    if (!state.abortController.signal.aborted) {
      state.abortController.abort(new DownloadCancelledError(vodId));
    }

    logger.info(`Cancellation requested for active VOD ${vodId}`);
    return true;
  }
  ...
  private async reconcileTaskState(taskId: number, client: PoolClient): Promise<void> {
  ...
  async downloadSegmentsInParallel(
    ...
    if (state.cancelRequested) throw new DownloadCancelledError(vodId);
    ...
    while ((!state.cancelRequested && queue.length > 0) || inProgress.size > 0) {
      while (!state.cancelRequested && queue.length > 0 && inProgress.size < maxConcurrent) {
        ...
        const downloadPromise = this.downloadSegment(vodId, segment, outputPath, throttleSpeed)
          ...
          .catch((error) => {
            inProgress.delete(downloadPromise);
            if (this.isCancellationError(error) || state.cancelRequested) {
              return;
            }
    ...
    if (state.cancelRequested) {
      throw new DownloadCancelledError(vodId);
    }
  }
  ...
  async downloadSegment(
    ...
    let responseStream: (NodeJS.ReadableStream & { destroy: (error?: Error) => void }) | null = null;
    ...
    try {
      ...
      responseStream = response.data as NodeJS.ReadableStream & { destroy: (error?: Error) => void };
      writer = fsSync.createWriteStream(outputPath, { flags: 'a' });
      state.openResources.add(responseStream);
      state.openResources.add(writer);
      ...
      const progressStream = new Transform({
        transform(chunk: Buffer, encoding: BufferEncoding, callback) {
          if (state.cancelRequested) {
            callback(new DownloadCancelledError(vodId));
            return;
          }
          ...
          if (state) {
            ...
            const remainingBytes = Math.max(0, state.progress.totalBytes - state.progress.bytesDownloaded);
            state.progress.eta = speed > 0
      ...
      let downloadStream: NodeJS.ReadableStream = responseStream;
      ...
      if (responseStream) {
        state.openResources.delete(responseStream);
      }
      if (writer) {
        state.openResources.delete(writer);
      }
    }
  }
  ...
  async processVOD(vod: any): Promise<void> {
    const client = await this.pool.connect();
    try {
      vod = await this.hydrateVODForDownload(vod, client);

      if (this.activeDownloads.has(vod.id)) {
        logger.info(`Skipping duplicate download start for active VOD ${vod.id}`);
        return;
      }
      ...
      if (isCompleted) {
        ...
        await this.reconcileTaskState(vod.task_id, client);
        return;
      }
      ...
      const downloadPathResolution = resolveDownloadPath(settings?.download_path);
      const downloadPath = downloadPathResolution.runtimePath;
      const tempPath = settings?.temp_dir || this.tempDir;
      const fileSystem = tempPath === this.tempDir
        ? this.fileSystem
        : new FileSystemManager(tempPath);
      const bandwidthLimit = 0;
      const concurrentLimit = settings?.concurrent_limit || 3;

      if (
        downloadPathResolution.usedRuntimeOverride
        && downloadPathResolution.configuredPath
        && downloadPathResolution.configuredPath !== downloadPath
      ) {
        logger.info(`Using runtime download path override "${downloadPath}" instead of configured path "${downloadPathResolution.configuredPath}"`);
      }
      ...
      const workingDir = await fileSystem.createTempDirectory(vod.twitch_id);
      ...
      try {
        ...
        const { outputPath: finalVideoPath, checksum } = await this.combineSegments(
          vod,
          workingDir,
          downloadPath,
        ...
        await client.query(`
          ...
          SET status              = CASE 
                                      ...
                                      ELSE 'running'
                                    END,
              status_message      = CONCAT('Downloaded: ', $2::int, '/', $3::int, ' VODs completed'),
              progress_percentage = $4::int,
              items_completed     = $2::int,
              items_total         = $3::int,
              updated_at          = NOW()
          WHERE task_id = $1
        `, [
        ...
        const wasCancelled = this.isCancellationError(error) || state.cancelRequested;
        ...
        if (wasCancelled) {
          ...
          this.webSocketService.emitDownloadStatusChange(
            vod.id,
            vod.task_id,
            'cancelled',
            Math.round(vod.download_progress || 0)
          );

          await this.reconcileTaskState(vod.task_id, client);
          logger.info(`Cancelled VOD download ${vod.id} cleanly`);
          return;
        }
        ...
        await client.query(`
          UPDATE vods
          SET status = $2::vod_status,
              download_status = $2::vod_status,
              error_message = $3,
              updated_at = NOW()
          WHERE id = $1
        ...
        await fileSystem.cleanupDirectory(workingDir);
  ...
  private async hydrateVODForDownload(vod: any, client: PoolClient): Promise<any> {
    if (!vod?.id) {
      throw new Error('VOD id is required to start a download');
    }

    const hasRequiredFields = vod.twitch_id && vod.task_id && vod.title;
    ...
    if (result.rows.length === 0) {
      throw new Error(`VOD ${vod.id} not found`);
    }

    const hydrated = { ...result.rows[0] };
    for (const [key, value] of Object.entries(vod)) {
  ...
  private async combineSegments(
    ...
    try {
      ...
      const title = typeof vod.title === 'string' && vod.title.trim()
        ? vod.title
        : `vod_${vod.twitch_id}`;
      ...
      // Concatenate .ts segments directly while hashing the same byte stream.
      // This avoids a second full-file checksum pass over slow host-mounted storage.
  ...
  private async updateDownloadProgress(vodId: number, totalSegments: number): Promise<void> {
    ...
    const progressPercentage = totalSegments > 0
      ...
      : 0;
    const downloadSpeed = Number.isFinite(state.progress.speed) && state.progress.speed > 0
      ? Math.round(state.progress.speed)
      : 0;
    const etaSeconds = Number.isFinite(state.progress.eta) && state.progress.eta > 0
      ? Math.round(state.progress.eta)
      : 0;
      ...
      try {
        ...
        await client.query(`
          ...
          SET download_progress = $2,
              ...
              updated_at = NOW()
          WHERE id = $1
        `, [vodId, progressPercentage, completedSegments, totalSegments, downloadSpeed, etaSeconds]);
