...
interface TaskRestrictionState {
  totalVodCount: number;
  totalStorageBytes: number;
  channelVodCounts: Map<number, number>;
  channelStorageBytes: Map<number, number>;
  channelFollowerCounts: Map<number, number>;
}

interface TaskChannelRef {
  twitchId: string;
  dbId: number;
}
...
export class TaskHandler extends EventEmitter {
  ...
  private async scanForVODs(task: any, client: any): Promise<void> {
    ...
    try {
      const taskChannels = this.getTaskChannelRefs(task);
      const conditions = this.normalizeTaskConditions(task.conditions);
      const restrictions = this.normalizeTaskRestrictions(task.restrictions);
      const restrictionState = await this.loadRestrictionState(taskId, client);
      ...
      if (taskChannels.length > 0) {
        totalChannels = taskChannels.length;
        logger.info(`TaskHandler: Processing ${totalChannels} channels for task ${taskId}`);

        for (let i = 0; i < taskChannels.length; i++) {
          const channelTwitchId = taskChannels[i].twitchId;
          const channelDbId = taskChannels[i].dbId;
          try {
            ...
            const channelConditionResult = await this.evaluateChannelConditions(
              channelDbId,
            ...
            if (!channelConditionResult.shouldQueue) {
              skippedVods += filteredVods.length;

              for (const vod of filteredVods) {
                await this.recordTaskSkip(
                  taskId,
                  vod.id,
                  channelConditionResult.code || 'CHANNEL_CONDITION',
                  channelConditionResult.details || {},
                  client
                );
              }

              logger.info(`TaskHandler: Skipping channel ${channelTwitchId} for task ${taskId}`, channelConditionResult.details);
              processedChannels++;
              continue;
            }
            ...
            for (const vod of filteredVods) {
              try {
                ...
                const candidateEvaluation = this.evaluateCandidateForQueueing(
                  vodInfo,
                  channelDbId,
                  preferredQuality,
                  conditions,
                ...
                if (!candidateEvaluation.shouldQueue) {
                  skippedVods++;
                  await this.recordTaskSkip(
                    taskId,
                    vod.id,
                    candidateEvaluation.code || 'TASK_CRITERIA',
                    candidateEvaluation.details || {},
                    client
                  );
                  continue;
                }
                ...
                this.applyCandidateToRestrictionState(
                  channelDbId,
                  candidateEvaluation.estimatedSizeBytes,
                  restrictionState
                );
                processedVods++;
  ...
  private checkGameMatch(title: string, gameNames: string[]): {
    ...
    for (const gameName of gameNames) {
      ...
      const gameNameVariants = this.getGameTitleVariants(gameName);

      for (const gameNameVariant of gameNameVariants) {
        const gameNameLower = gameNameVariant.toLowerCase();
        ...
        if (gameAppears) {
          ...
          if (contextResult.isActualGameplay) {
            return {
              ...
              details: gameNameVariant === gameName
                ? contextResult.reason
                : `${contextResult.reason}; matched title alias "${gameNameVariant}"`
            };
          } else {
            // Game appears but context suggests it's not being played
            logger.info(`Game "${gameName}" appears in title as "${gameNameVariant}" but context suggests it's not being played: ${contextResult.reason}`);
          }
        }
      }
  ...
  /**
   * Common title variants streamers use for games whose Twitch category name is formal.
   */
  private getGameTitleVariants(gameName: string): string[] {
    const variants = new Set<string>([gameName]);
    const normalized = gameName.toLowerCase().trim();

    if (normalized === 'diablo iv' || normalized === 'diablo 4') {
      [
        'Diablo IV',
        'Diablo 4',
        'D4',
        // Observed Lethalfrag typo in archived Twitch VOD titles.
        'Daiblo IV',
        'Daiblo 4'
  ...
  private getTaskChannelRefs(task: any): TaskChannelRef[] {
    const twitchIds = Array.isArray(task.channel_twitch_ids) ? task.channel_twitch_ids : [];
    const dbIds = Array.isArray(task.channel_db_ids) ? task.channel_db_ids : [];

    return twitchIds.map((twitchId: unknown, index: number) => ({
      twitchId: twitchId != null ? String(twitchId) : '',
      dbId: Number(dbIds[index]),
    })).filter((channel: TaskChannelRef) => channel.twitchId && Number.isFinite(channel.dbId) && channel.dbId > 0);
  }

  private normalizeTaskConditions(value: any): TaskConditions {
    if (!value) {
      return {};
    }
  ...
  private normalizeTaskRestrictions(value: any): TaskRestrictions {
    if (!value) {
      return {};
    }
  ...
  private async loadRestrictionState(taskId: number, client: PoolClient): Promise<TaskRestrictionState> {
    const result = await client.query(`
      SELECT
        v.channel_id,
        COUNT(*) FILTER (
          WHERE COALESCE(v.download_status, 'pending') NOT IN ('failed', 'cancelled')
        )::int as vod_count,
        COALESCE(SUM(
          CASE
            WHEN COALESCE(v.download_status, 'pending') NOT IN ('failed', 'cancelled')
              THEN COALESCE(cv.file_size_bytes, v.file_size, v.estimated_size, 0)
            ELSE 0
          END
    ...
    const state: TaskRestrictionState = {
      totalVodCount: 0,
      totalStorageBytes: 0,
    ...
    for (const row of result.rows) {
      ...
      const vodCount = Number(row.vod_count) || 0;
      const storageBytes = Number(row.storage_bytes) || 0;

      if (Number.isFinite(channelId) && channelId > 0) {
        state.channelVodCounts.set(channelId, vodCount);
        state.channelStorageBytes.set(channelId, storageBytes);
      }

      state.totalVodCount += vodCount;
      state.totalStorageBytes += storageBytes;
    }

    return state;
  }

  private async evaluateChannelConditions(
    channelDbId: number,
    channelTwitchId: string,
    conditions: TaskConditions,
    restrictionState: TaskRestrictionState,
    client: PoolClient
  ): Promise<{ shouldQueue: boolean; code?: string; details?: Record<string, any> }> {
    if (!conditions.minFollowers) {
      return { shouldQueue: true };
    }

    const followerCount = await this.resolveChannelFollowerCount(
      channelDbId,
      channelTwitchId,
      restrictionState,
      client
    ...
    if (followerCount < conditions.minFollowers) {
      return {
        shouldQueue: false,
        code: 'MIN_FOLLOWERS',
        details: {
          ...
          channelTwitchId,
          followerCount,
          requiredFollowers: conditions.minFollowers,
        },
      };
    }
  ...
  private async resolveChannelFollowerCount(
    channelDbId: number,
    channelTwitchId: string,
    restrictionState: TaskRestrictionState,
    client: PoolClient
  ): Promise<number> {
    const cachedCount = restrictionState.channelFollowerCounts.get(channelDbId);
    if (cachedCount !== undefined) {
      return cachedCount;
    }

    const existingFollowerResult = await client.query(`
      SELECT follower_count
      FROM channels
      WHERE id = $1 AND is_active = true
    `, [channelDbId]);

    let followerCount = Number(existingFollowerResult.rows[0]?.follower_count) || 0;

    if (followerCount <= 0) {
      followerCount = await this.twitchService.getChannelFollowers(channelTwitchId);
      ...
      if (followerCount > 0) {
        await client.query(`
          ...
          SET follower_count = $2,
              updated_at = NOW()
          WHERE id = $1
        `, [channelDbId, followerCount]);
      }
    }

    restrictionState.channelFollowerCounts.set(channelDbId, followerCount);
    return followerCount;
  }

  private evaluateCandidateForQueueing(
    vod: TwitchVOD,
    channelId: number,
    ...
    const durationSeconds = this.parseVodDurationToSeconds(vod.duration);
    const normalizedLanguage = (vod.language || '').toLowerCase();
    const allowedLanguages = (conditions.languages || []).map((language) => language.toLowerCase());
    const estimatedSizeBytes = this.estimateVodSizeBytes(durationSeconds, preferredQuality);

    if (conditions.minViews && (vod.view_count || 0) < conditions.minViews) {
      return {
        shouldQueue: false,
        code: 'MIN_VIEWS',
        details: {
          actualViews: vod.view_count || 0,
          requiredViews: conditions.minViews,
        },
        estimatedSizeBytes,
      };
    }

    if (conditions.minDuration && durationSeconds < conditions.minDuration * 60) {
      return {
        shouldQueue: false,
        code: 'MIN_DURATION',
        details: {
          actualDurationSeconds: durationSeconds,
          requiredDurationMinutes: conditions.minDuration,
        },
        estimatedSizeBytes,
      };
    }

    if (allowedLanguages.length > 0 && (!normalizedLanguage || !allowedLanguages.includes(normalizedLanguage))) {
      return {
        shouldQueue: false,
        code: 'LANGUAGE_FILTER',
        details: {
          actualLanguage: normalizedLanguage || null,
          allowedLanguages,
        },
        estimatedSizeBytes,
      };
    ...
    const currentChannelVodCount = restrictionState.channelVodCounts.get(channelId) || 0;
    const currentChannelStorage = restrictionState.channelStorageBytes.get(channelId) || 0;

    if (restrictions.maxVodsPerChannel && currentChannelVodCount + 1 > restrictions.maxVodsPerChannel) {
      return {
        shouldQueue: false,
        code: 'MAX_VODS_PER_CHANNEL',
        details: {
          channelId,
          currentVodCount: currentChannelVodCount,
          maxVodsPerChannel: restrictions.maxVodsPerChannel,
        },
        estimatedSizeBytes,
      };
    }

    if (restrictions.maxTotalVods && restrictionState.totalVodCount + 1 > restrictions.maxTotalVods) {
      return {
        shouldQueue: false,
        code: 'MAX_TOTAL_VODS',
        details: {
          currentVodCount: restrictionState.totalVodCount,
          maxTotalVods: restrictions.maxTotalVods,
        },
        estimatedSizeBytes,
      };
    }
    ...
    if (
      restrictions.maxStoragePerChannel
      && currentChannelStorage + estimatedSizeBytes > this.gigabytesToBytes(restrictions.maxStoragePerChannel)
    ) {
      return {
        shouldQueue: false,
        code: 'MAX_STORAGE_PER_CHANNEL',
        details: {
          channelId,
          currentStorageBytes: currentChannelStorage,
          estimatedSizeBytes,
          maxStoragePerChannelGb: restrictions.maxStoragePerChannel,
        },
        estimatedSizeBytes,
      };
    }
    ...
    if (
      restrictions.maxTotalStorage
      && restrictionState.totalStorageBytes + estimatedSizeBytes > this.gigabytesToBytes(restrictions.maxTotalStorage)
    ) {
      return {
        shouldQueue: false,
        code: 'MAX_TOTAL_STORAGE',
        details: {
          currentStorageBytes: restrictionState.totalStorageBytes,
          estimatedSizeBytes,
          maxTotalStorageGb: restrictions.maxTotalStorage,
        },
        estimatedSizeBytes,
      };
    }
  ...
  private applyCandidateToRestrictionState(
    channelId: number,
    estimatedSizeBytes: number,
    restrictionState: TaskRestrictionState
  ): void {
    restrictionState.totalVodCount += 1;
    restrictionState.totalStorageBytes += estimatedSizeBytes;
    restrictionState.channelVodCounts.set(channelId, (restrictionState.channelVodCounts.get(channelId) || 0) + 1);
    restrictionState.channelStorageBytes.set(
      channelId,
      (restrictionState.channelStorageBytes.get(channelId) || 0) + estimatedSizeBytes
    );
  }

  private parseVodDurationToSeconds(duration: string | null | undefined): number {
    if (!duration) {
      return 0;
    }
  ...
  private estimateVodSizeBytes(durationSeconds: number, preferredQuality: string): number {
    const bitrateByQuality: Record<string, number> = {
      source: 6_000_000,
      '1080p60': 6_000_000,
      '1080p': 4_500_000,
      '720p60': 3_500_000,
    ...
    const bitrate = bitrateByQuality[preferredQuality] || bitrateByQuality.source;
    const bytesPerSecond = bitrate / 8;

    return Math.ceil(durationSeconds * bytesPerSecond * 1.1);
  }
