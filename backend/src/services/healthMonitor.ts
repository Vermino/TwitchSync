...
export class HealthMonitorService extends EventEmitter {
  ...
  private async checkDiskSpace(): Promise<HealthCheck> {
    try {
      ...
      try {
        await fs.mkdir(config.storage.storagePath, { recursive: true });
        if (fs.statfs) {
      ...
      if (totalBytes <= 0) {
        throw new Error(`Unable to determine disk capacity for ${config.storage.storagePath}`);
      }
  ...
  private async getFallbackDiskSpace(checkPath: string): Promise<{ total: number; free: number }> {
    try {
      if (process.platform === 'win32') {
        ...
        const { stdout } = await execAsync(`df -B1 "${resolvedPath}"`);
        const lines = stdout.trim().split('\n');
      ...
      logger.warn(`Could not determine real disk space via fallback for ${checkPath}, using safe defaults`);
      return {
