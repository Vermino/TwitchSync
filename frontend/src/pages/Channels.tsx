...
const getChannelDisplayName = (channel: Partial<Channel> & { username?: string }): string => {
  return channel.display_name?.trim()
    || channel.username?.trim()
    || 'Unknown channel';
};

const getChannelUsername = (channel: Partial<Channel> & { username?: string }): string => {
  return channel.username?.trim()
    || getChannelDisplayName(channel).toLowerCase().replace(/\s+/g, '')
    || 'unknownchannel';
};

const getChannelInitial = (channel: Partial<Channel> & { username?: string }): string => {
      ...
      for (const channel of selectedChannels) {
        try {
          const result = await api.createChannel({
            ...
            profile_image_url: channel.profile_image_url || channel.thumbnail_url,
            follower_count: channel.follower_count || 0,
            description: channel.description || ''
          });
          results.push(result);
          ...
          if (error.message?.includes('already exists')) {
            console.warn(`Channel ${getChannelDisplayName(channel)} already exists, skipping...`);
            continue;
          }
                  ...
                  <div className="flex items-center">
                    <div className="relative">
                      <Avatar className="h-10 w-10">
                        <AvatarImage src={channel.profile_image_url || undefined} alt={getChannelDisplayName(channel)} />
                        <AvatarFallback>{getChannelInitial(channel)}</AvatarFallback>
                      </Avatar>
                      {channel.is_live && (
                    ...
                    <div className="ml-4">
                      <div className="text-sm font-medium text-gray-900">{getChannelDisplayName(channel)}</div>
                      <div className="text-sm text-gray-500">@{getChannelUsername(channel)}</div>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
