...
const TaskModal: React.FC<TaskModalProps> = ({
  ...
  const [formData, setFormData] = useState({
    name: '',
    description: '',
    task_type: 'channel' as const,  // Add explicit type
    channel_ids: [] as number[],
    game_ids: [] as number[],
    schedule_type: 'interval' as const,  // Add explicit type
    schedule_value: '3600',
    storage_limit_gb: '',
    retention_days: '',
    auto_delete: false,
    priority: 'low' as const,  // Add explicit type
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Determine task type based on selected items
    let taskType = 'channel';
    if (formData.channel_ids.length > 0 && formData.game_ids.length > 0) {
      taskType = 'combined';
    } else if (formData.game_ids.length > 0) {
      taskType = 'game';
    }

    // Ensure required fields have default values
    const submissionData = {
      name: formData.name.trim() || 'Untitled Task',  // Provide default name
      description: formData.description,
      task_type: taskType as 'channel' | 'game' | 'combined',
      channel_ids: formData.channel_ids,
      game_ids: formData.game_ids,
      schedule_type: formData.schedule_type || 'interval',
      schedule_value: formData.schedule_value || '3600',
      storage_limit_gb: formData.storage_limit_gb ? parseInt(formData.storage_limit_gb) : undefined,
      retention_days: formData.retention_days ? parseInt(formData.retention_days) : undefined,
      auto_delete: Boolean(formData.auto_delete),
    ...
    if (!submissionData.channel_ids.length && !submissionData.game_ids.length) {
      alert('Please select at least one channel or game');
      return;
    }
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full flex flex-col">
              ...
              <div className="flex-1 overflow-hidden">
                <TabsContent value="channels" className="h-[460px] overflow-auto mt-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-h-[400px] overflow-y-auto p-1 mt-4">
                    {channelsLoading ? (
                ...
                <TabsContent value="games" className="h-[460px] overflow-auto mt-4">
                  ...
                  <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-4 max-h-[400px] overflow-y-auto p-1 mt-4">
                    {gamesLoading ? (
                    ...
                    <div className="space-y-4">
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onValueChange={(value: 'interval' | 'cron' | 'manual') => setFormData(prev => ({
                            ...prev,
                            schedule_type: value,
                    ...
                    <div className="space-y-4">
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onValueChange={(value: 'low' | 'normal' | 'high') => setFormData(prev => ({
                            ...prev,
                            priority: value
                          }))}
                        >
