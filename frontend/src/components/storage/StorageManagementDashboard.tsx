...
export default function StorageManagementDashboard() {
  ...
  const unprotectFileMutation = useMutation({
    mutationFn: (vodId: string) => api.unprotectVod(vodId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['largest-files'] });
      queryClient.invalidateQueries({ queryKey: ['storage-analytics'] });
    },
  ...
  const handleBulkProtect = useCallback(async () => {
    try {
      ...
      const selectedFileItems = largestFiles?.files?.filter((f: FileItem) => selectedFiles.includes(f.id));
  ...
  return (
    <div className="space-y-6 p-6" data-testid="storage-dashboard">
      {/* Header */}
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid gap-4 md:grid-cols-3">
                <Button
                  data-testid="storage-verify-all"
                            ...
                            <div className="flex items-center justify-end gap-1">
                              <TooltipProvider>
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Button
                                      data-testid={`storage-verify-file-${file.vodId}`}
                                      variant="ghost"
                                      size="sm"
                              ...
                              <TooltipProvider>
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Button
                                      data-testid={`storage-redownload-file-${file.vodId}`}
                                      variant="ghost"
                                      size="sm"
                              ...
                              <TooltipProvider>
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Button
                                      data-testid={`storage-protect-file-${file.vodId}`}
                                      variant="ghost"
                                      size="sm"
                                      onClick={() => {
                                        if (file.isProtected) {
                                          unprotectFileMutation.mutate(file.vodId, {
                                            onSuccess: () => {
                                              toast({
                                      ...
                                      disabled={protectFileMutation.isPending || unprotectFileMutation.isPending}
                                    >
                                      {file.isProtected ? (
                                        <Shield className="h-4 w-4 text-blue-600" />
                                      ) : (
