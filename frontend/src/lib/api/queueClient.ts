...
export class QueueClient extends BaseApiClient {
  ...
  async getQueueStats(): Promise<QueueStats> {
    try {
      ...
      return {
        total_items: (response.data.pending || 0) + (response.data.queued || 0) + (response.data.downloading || 0),
        pending: response.data.pending || 0,
        downloading: response.data.downloading || 0,
        completed: response.data.completed || 0,
        failed: response.data.failed || 0,
        paused: response.data.paused || 0,
        total_queue_size: response.data.total_size_bytes || 0,
        total_download_speed: response.data.avg_download_speed || 0,
      };
    } catch (error) {
  ...
  async pauseItem(itemId: string): Promise<void> {
    try {
      await this.axios.post(
        ...
        { type: 'pause_all', target: 'selected', item_ids: [itemId] },
        { headers: this.getHeaders() }
      );
    } catch (error) {
  ...
  async resumeItem(itemId: string): Promise<void> {
    try {
      await this.axios.post(
        `${this.baseURL}/queue/bulk`,
        { type: 'resume_all', target: 'selected', item_ids: [itemId] },
        { headers: this.getHeaders() }
      );
    } catch (error) {
  ...
  async cancelItem(itemId: string): Promise<void> {
    try {
      await this.axios.post(
        `${this.baseURL}/downloads/cancel/${itemId}`,
        {},
        { headers: this.getHeaders() }
      );
    } catch (error) {
  ...
  async reorderQueue(itemId: string, newPosition: number): Promise<void> {
    try {
      await this.axios.post(
        `${this.baseURL}/queue/${itemId}/${newPosition <= 1 ? 'move-top' : 'move-bottom'}`,
        {},
        { headers: this.getHeaders() }
      );
    } catch (error) {
