...
export class TwitchAuth {
  constructor(_config: TwitchAuthConfig) {}
  ...
  public async getAuthUrl(): Promise<{ url?: string; error?: string; instructions?: string; redirectTo?: string }> {
    try {
      ...
      logger.error('Failed to get auth URL from backend:', error);

      return {
        error: 'Authentication service unavailable',
        instructions: 'The backend could not generate a Twitch login URL. Verify the backend is healthy and try again.'
      };
    }
  }
